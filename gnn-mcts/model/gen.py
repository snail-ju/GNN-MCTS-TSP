...
class EdgeConv(MessagePassing):
    ...
    def forward(self, original_node_attr, node_attr, edge_index, edge_attr):
        row, col = edge_index
        ...
        node_attr = self.lin_node(original_node_attr)
    ...
    def update(self, aggr_out, x, edge_attr):
        return self.activation(x + self.bn_neighbor(aggr_out) + edge_attr)
...
class GNN(nn.Module):
    def __init__(self, node_dim, edge_dim, embed_dim, layer):
        ...
        # Layers
        # share parameter
        lin_node = nn.Linear(node_dim, embed_dim)
        ...
        self.gnn_layers = nn.ModuleList(
            [EdgeConv(lin_node, lin_edge, lin_neighbor_first, bn_node, bn_edge, bn_neighbor, activation)])
        for _ in range(1, layer):
            self.gnn_layers.append(
                EdgeConv(lin_node, lin_edge, lin_neighbor_other, bn_node, bn_edge, bn_neighbor, activation))

    def forward(self, node_attr, edge_index, edge_attr):
        original_node_attr = node_attr.clone()
        for i in range(self.layer):
            node_attr = self.gnn_layers[i](original_node_attr, node_attr, edge_index, edge_attr)
